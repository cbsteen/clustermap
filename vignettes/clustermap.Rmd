...
Sometimes, it is useful to apply the clustering estimated using one dataset, to another dataset of the same size, for example is one wishes to follow patient samples over time, using the original subgrouping identified in the first samples. This is done by plotting a heatmap using another dataset with the `draw.hmap()` function, right after having performed the clustering on the original dataset with `hcluster()`. 
