...
###Silhouette plot
Silhouette plots are used to assess the quality of the subclustering performed on the data. A silhouette plot allows visual inspection of the resulting number of clusters from subclustering, by providing a measure of how close each point in one cluster is to points in neighboring clusters. Using the same clustering as in example 12, in the following example we show the silhouette plot of the column wise clustering. 
...
###Gap statistics
The Gap statistics is a measure of clustering quality. Using the `draw.gap()` function, the user can plot the Gap statistics and use it to get an idea the best number of clusters. The number of clusters (depicted on the x-axis) should be selected to maximize the Gap statistic. 
